...
import org.daypilot.data.EventVersion;
...
import org.daypilot.demo.db.Row;
import org.daypilot.demo.db.Table;
...
public class DpsServlet extends HttpServlet {
	...
	public class Dps extends DayPilotScheduler {
		...
		public void onEventMove(EventMoveArgs ea) throws Exception {
			...
			// update the DB
			if ("dps_joint".equals(getId())) { // joint events/multimove, update
												// all moved events
				for (EventMoveArgs item : ea.getMultimove()) {
					Db.moveEvent(getRequest(), item.getValue(), item.getNewStart().toTimeStamp(),
							item.getNewEnd().toTimeStamp(), item.getNewResource());
				}
			} else {
		...
		public void onTimeRangeSelected(TimeRangeSelectedArgs ea) throws Exception {
			Db.insertEvent(getRequest(), "New event", ea.getStart().toTimeStamp(), ea.getEnd().toTimeStamp(),
					ea.getResource(), false);
			update();
		}
		...
		public void onCommand(CommandArgs ea) throws Exception {
			...
			} else if ("selected".equals(ea.getCommand())) {
				updateWithMessage("Number of selected events: " + getMultiselect().size(), UpdateType.EVENTS_ONLY);
			}
		}
		...
		public void onBeforeResHeaderRender(BeforeResHeaderRenderArgs ea) {
			if (ea.isCorner()) {
				String c = String.format(
						"<div style='padding:5px; font-weight: bold; font-size:22px; text-align:center'>%s</div>",
						getStartDate().getYear());
				ea.setInnerHTML(c);
			}
			if ("dps_areas".equals(getId())) {
				ea.getAreas().add(new Area().width(17).bottom(1).right(0).top(0).cssClass("resource_action_menu")
						.html("<div><div></div></div>").javaScript("alert(e.Value);"));
			}
		}
		...
		public void onBeforeCellRender(BeforeCellRenderArgs ea) {
			...
			if ("dps_cells".equals(getId())) {
				if (ea.getStart().getDayOfWeek() == DayOfWeek.SUNDAY
						|| ea.getStart().getDayOfWeek() == DayOfWeek.SATURDAY) {
		...
		public void onBeforeEventRender(BeforeEventRenderArgs ea) {
			...
			if ("dps_areas".equals(getId())) {
				ea.getAreas().add(new Area().width(17).bottom(9).right(2).top(3).cssClass("event_action_delete")
						.javaScript("dps.commandCallBack('delete', {id:e.value() });"));
				ea.getAreas().add(new Area().width(17).bottom(9).right(19).top(3).cssClass("event_action_menu")
						.contextMenu("cmSpecial"));
			}
			...
			if (isShort) {
				int w = 2;
				ea.setEventResizeEnabled(false);
				ea.getAreas().add(new Area().left(0).top(0).bottom(0).width(w).resizeStart());
				ea.getAreas().add(new Area().right(0).top(0).bottom(0).width(w).resizeEnd());
			}
			
			EventVersion v = new EventVersion();
			v.setStart(ea.getStart());
			v.setEnd(ea.getEnd());
			v.setText("Version");
			ea.getVersions().add(v);
		...
		public void onFinish() throws Exception {
			...
			// reload events
			Table events = Db.getEvents(getRequest(), getStartDate().toDate(),
					getStartDate().addDays(getDays()).toDate());
			setEvents(events);
